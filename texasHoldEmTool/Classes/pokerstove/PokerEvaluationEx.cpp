...
unsigned char PokerEvaluationEx::cardType_Evaluation(unsigned char cbCardData[], unsigned char cbCardCount)
{
	...
	unsigned char cbFirstColor = PokerEvaluation::cardColor_Evaluation(cbCardData[0]);
	unsigned char cbFirstValue = PokerEvaluation::cardLogicValue_Evaluation(cbCardData[0]);
    
	//牌形分析
	for (unsigned char i=1;i < cbCardCount;i++)
	{
		//数据分析
		if ( PokerEvaluation::cardColor_Evaluation(cbCardData[i])!=cbFirstColor) cbSameColor=0;
		if (cbFirstValue!=( PokerEvaluation::cardLogicValue_Evaluation(cbCardData[i])+i)) bLineCard=0;
	...
	if((bLineCard == 0)&&(cbFirstValue == 14))
	{
		...
		{
			unsigned char cbLogicValue = PokerEvaluation::cardLogicValue_Evaluation(cbCardData[i]);
	...
	//皇家同花顺
	if ((cbSameColor==1)&&(bLineCard==1)&&( PokerEvaluation::cardLogicValue_Evaluation(cbCardData[1]) ==13 ))
...
int PokerEvaluationEx::compare_Evaluation(unsigned char cbFirstData[], unsigned char cbNextData[], unsigned char cbCardCount)
{
	...
	switch(cbFirstType)
	{
        case CT_SINGLE:			//单牌
		{
			...
			{
				unsigned char cbNextValue = PokerEvaluation::cardLogicValue_Evaluation(cbNextData[i]);
				unsigned char cbFirstValue = PokerEvaluation::cardLogicValue_Evaluation(cbFirstData[i]);
		...
		{
			//数值判断
			unsigned char cbNextValue = PokerEvaluation::cardLogicValue_Evaluation(cbNextData[0]);
			unsigned char cbFirstValue = PokerEvaluation::cardLogicValue_Evaluation(cbFirstData[0]);
            
			unsigned char bFirstmin= (cbFirstValue ==(PokerEvaluation::cardLogicValue_Evaluation(cbFirstData[1])+9));
			unsigned char bNextmin= (cbNextValue ==(PokerEvaluation::cardLogicValue_Evaluation(cbNextData[1])+9));

			//大小顺子
			if ((bFirstmin==1)&&(bNextmin == 0))
			{
                if ( PokerEvaluation::cardValue_Evaluation( cbFirstData[0] ) == 1 && PokerEvaluation::cardValue_Evaluation( cbNextData[0] ) != 13 &&  PokerEvaluation::cardValue_Evaluation( cbNextData[0] ) != 1 ) {
                    return 1;
                }
			...
			{
                if (  PokerEvaluation::cardValue_Evaluation( cbNextData[0] ) == 1 && PokerEvaluation::cardValue_Evaluation( cbFirstData[0] ) != 13 &&  PokerEvaluation::cardValue_Evaluation( cbFirstData[0] ) != 1 ){
                    return -1;
                }
				return 1;
			}
...
void PokerEvaluationEx::analysebCardData_Evaluation(const unsigned char cbCardData[], unsigned char cbCardCount, tagAnalyseResult *AnalyseResult)
{
	...
	for (unsigned char i=0;i<cbCardCount;i++)
	{
		...
		{
			//逻辑对比
			if ( PokerEvaluation::cardLogicValue_Evaluation(cbCardData[j])!=cbLogicValue) break;
...
int PokerEvaluationEx::cardTypeTry_Evaluation(unsigned char cbCardData[], unsigned char cbCardCount)
{
    ...
    PokerEvaluation::sortCardList_Evaluation( cbCardData_tag, cbCardCount);
    ...
    for ( int i = 0; i < 5; i++) {
        if ( i < cbCardCount) {
            value[i] = PokerEvaluation::cardValue_Evaluation( cbCardData[i] );
            color[i] = PokerEvaluation::cardColor_Evaluation( cbCardData[i] );
...
//6返5
unsigned char PokerEvaluationEx::FiveFromSix(unsigned char cbHandCardData[],unsigned char cbHandCardCount,unsigned char cbCenterCardData[],unsigned char cbCenterCardCount,unsigned char cbLastCardData[],unsigned char cbLastCardCount){
    //临时变量
	unsigned char cbTempCardData[6],cbTempLastCardData[5];
	...
	memcpy(&cbTempCardData[2],cbCenterCardData,sizeof(unsigned char)*4);
    
	//排列扑克
	PokerEvaluation::sortCardList_Evaluation(cbTempCardData,CountArray(cbTempCardData));
    
	memcpy(cbLastCardData,cbTempCardData,sizeof(unsigned char)*MAX_CENTERCOUNT);
	unsigned char cbCardKind = cardType_Evaluation(cbLastCardData,sizeof(unsigned char)*MAX_CENTERCOUNT);
			...
			for (int k = j+1;k<4;k++)
			{
				for (int l =k+1;l<5;l++)
				{
					for (int m=l+1;m<6;m++)
					{
						...
						//获取牌型
						cbTempCardKind = cardType_Evaluation(cbTempLastCardData,CountArray(cbTempLastCardData));
                        
						//牌型大小
						if(compareEx_Evaluation(cbTempLastCardData,cbLastCardData,CountArray(cbTempLastCardData))==2)
						{
...
//7返5
unsigned char PokerEvaluationEx::FiveFromSeven(unsigned char cbHandCardData[],unsigned char cbHandCardCount,unsigned char cbCenterCardData[],unsigned char cbCenterCardCount,unsigned char cbLastCardData[],unsigned char cbLastCardCount){
    //临时变量
...
void PokerEvaluationEx::maxIdx_Evaluation(unsigned char handCardData[][2], unsigned char curCenterCardData[], int iPlays, int iJoinNum, int *maxIdx){
    
    for (int idx = 0; idx < iPlays; idx++) {
        maxIdx[ idx ] = -1;
    }

    unsigned char cbLastCardData[ iPlays ][ 7 ];
    ...
    for (int idx = 0; idx < iPlays; idx++) {
        ...
        if ( iJoinNum > 5) {
            ...
            if ( iJoinNum == 6) {
                FiveFromSix( handCardData[idx], 2, curCenterCardData, iJoinNum-2, cbLastCardData[ idx ], 5);
            }else{
                FiveFromSeven( handCardData[idx], 2, curCenterCardData, iJoinNum-2, cbLastCardData[ idx ], 5);
            }
        }else{
            
            cbLastCardData[ idx ][0] = handCardData[ idx ][0];
            cbLastCardData[ idx ][1] = handCardData[ idx ][1];
            
            for (int j = 0; j < iJoinNum-2; j++) {
                cbLastCardData[ idx ][ j+2 ] = curCenterCardData[j];
            }
        }
    }
    
    iJoinNum = iJoinNum>5?5:iJoinNum;
    
    //比较大小
    //求出最大的
    int iSelectMax = 0;
    for (int idx = 0; idx < iPlays-1; idx++) {
        if (-1 == compareEx_Evaluation( cbLastCardData[ idx ], cbLastCardData[ idx+1 ], iJoinNum) ) {
            iSelectMax = idx+1;
        }
    }
    ...
    //检测与最大同的数据
    int itt = 0;
    maxIdx[ itt++ ] = iSelectMax;
    for (int idx = 0; idx < iPlays; idx++) {
        if ( idx == iSelectMax ) {
            continue;
        }
        if (0 == compareEx_Evaluation( cbLastCardData[ idx ], cbLastCardData[ iSelectMax ], iJoinNum) ) {
            maxIdx[ itt++ ] = idx;
        }
    }
}